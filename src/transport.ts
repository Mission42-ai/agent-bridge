...
// ---------------------------------------------------------------------------
// Normalize incoming data to ExecutionRequest
// ---------------------------------------------------------------------------

export function normalizeRequest(data: Record<string, unknown>): ExecutionRequest {
...
function runRequest(request: ExecutionRequest): void {
  activeCount++;
  console.log(
    `[Queue] Executing ${request.id} (active: ${activeCount}/${config.maxConcurrent}, queued: ${queue.length})`,
  );
  ...
  executeRequest(request)
    ...
    .finally(() => {
      activeCount--;
      console.log(
        `[Queue] Finished ${request.id} (active: ${activeCount}/${config.maxConcurrent}, queued: ${queue.length})`,
      );

      if (queue.length > 0) {
        const next = queue.shift()!;
        console.log(
          `[Queue] Dequeuing ${next.id} (${queue.length} still queued)`,
        );
        runRequest(next);
      }
    });
...
console.log(
  `Listening on "${PUSHER_CHANNEL}" for "${PUSHER_EVENT}" (max concurrent: ${config.maxConcurrent})...`,
);
...
channel.bind(PUSHER_EVENT, (data: Record<string, unknown>) => {
  ...
  if (activeCount >= config.maxConcurrent) {
    queue.push(request);
    console.log(
      `[Queue] Enqueued ${request.id} — at capacity (active: ${activeCount}/${config.maxConcurrent}, queued: ${queue.length})`,
    );
  } else {
    runRequest(request);
  }
